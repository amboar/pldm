    ...
    [[maybe_unused]] const uint32_t& record_handle)
{
    ...
    if (oemPlatformHandler && isHBRange(record_handle))
    {
        ...
        int rc = pldm_pdr_add_check(repo, pdr.data(), size, true, 0xFFFF, &applied_handle);
        if (rc) {
    ...
    if (merged)
    {
        ...
        else
        {
            ...
            if (isHostUp() || (terminus_handle & 0x8000))
            {
                ...
                int rc = pldm_entity_association_pdr_add_from_node_with_record_handle(
                    node, repo, &entities, numEntities, true, TERMINUS_HANDLE,
                    0xFFFFFFFF);
                if (rc)
                {
                    std::cerr << "Failed to add entity association PDR from node: " <<  rc << std::endl;
                }
            }
            else
            {
